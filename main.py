...
def get_playlists_data() -> list[list[str]]:
    ...
    data = pd.read_csv('/content/playlist-generator-model/data/spotify_playlists.csv')
    data = data.dropna()
...
def save_frequent_itemsets(rules: dict[str, set[str]]):
    ...
    with open('/content/playlist-generator-model/data/playlists_rules.pkl', 'wb') as f:
        pickle.dump(rules, f)
